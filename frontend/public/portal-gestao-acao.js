(()=>{
  ...
  function fixBanners(){document.querySelectorAll('img').forEach(img=>{const alt=(img.alt||'').toLowerCase();const near=((img.parentElement?.className||'')+' '+(img.parentElement?.parentElement?.className||'')).toLowerCase();if(alt.includes('seja')||alt.includes('banner')||near.includes('slide')||near.includes('banner')){img.loading='eager';img.style.display='block';img.style.width='100%';img.style.height='100%';img.style.objectFit='cover';img.style.opacity='1';img.style.visibility='visible';}})}
  function run(){fixBanners();const homeCard=[...document.querySelectorAll('button')].find(b=>(b.textContent||'').includes('Transparência')||(b.textContent||'').includes('Acompanhe as ações'));if(homeCard){swapIcon(homeCard);if(homeCard.dataset.ga!=='1'){homeCard.dataset.ga='1';[...homeCard.querySelectorAll('div')].forEach(d=>{const t=(d.textContent||'').trim();if(t==='Transparência')d.textContent='Gestão em Ação';if(t.includes('receitas')||t.includes('despesas'))d.textContent='Acompanhe as ações realizadas pela administração.'});}}const h=[...document.querySelectorAll('h1,h2,h3')].find(x=>(x.textContent||'').trim()==='Transparência');if(!h)return;const box=h.closest('.fadeIn')||h.parentElement;if(box&&box.dataset.gaPage!=='1'){box.dataset.gaPage='1';render(box)}}
  setInterval(run,400);
})();