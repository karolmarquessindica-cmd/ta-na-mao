(()=>{
  ...
  function key(){return 'tnm_gestao_acao_feed_'+(portalPayload?.condominio?.id||'sem-condominio')}
  ...
  function post(item){
    ...
    const n=document.createElement('div');n.textContent=portalPayload?.condominio?.nome||'Administração';n.style.cssText='font-size:14px;font-weight:950;color:#0C140D';
  ...
  function addButton(){
    ...
    const root=document.querySelector('.pnav')||document.querySelector('[class*="portal"]')||document.querySelector('#root');if(!root)return;
    const btn=document.createElement('button');btn.dataset.gestaoAcaoButton='1';btn.type='button';btn.textContent='Gestão em Ação';btn.style.cssText='width:100%;border:0;background:linear-gradient(135deg,#0B6B3A,#14884E);color:#fff;border-radius:18px;padding:14px 16px;font-size:15px;font-weight:950;box-shadow:0 14px 30px rgba(11,107,58,.22);margin:12px 0';btn.onclick=buildModal;
    const home=document.querySelector('.fadeIn')||root.parentElement||root;const after=home.querySelector('.cslide')?.parentElement;if(after&&after.parentElement===home)after.insertAdjacentElement('afterend',btn);else home.prepend(btn);
  }
