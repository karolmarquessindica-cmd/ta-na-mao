...
function Dashboard() {
  const [draft, setDraft] = useState(dashboardDefaults);
  const [applied, setApplied] = useState(dashboardDefaults);
  const [printData, setPrintData] = useState(null);
  const [printInfo, setPrintInfo] = useState("");
  const query = dashboardQuery(applied);
  const { data, loading } = useFetch(`/dashboard?${query}`, [query]);

  const stats = data?.stats || {};
  const filters = data?.filters || {};
  const condominios = filters.condominios || [];
  const categorias = filters.categorias || [{ value:"all", label:"Todas as categorias" }];
  const graficos = data?.graficos || {};
  const alertas = data?.alertas || [];
  ...
  function limpar() {
    setDraft(dashboardDefaults);
    setApplied(dashboardDefaults);
  }
  ...
  function handlePrintDashboard() {
    if (!data) return;
    setPrintData(data);
    setPrintInfo("Relatorio preparado. Na janela de impressao, escolha Salvar como PDF.");
    requestAnimationFrame(() => {
      setTimeout(() => {
        try {
          window.print();
        } catch {
          setPrintInfo("Nao foi possivel abrir a impressao automaticamente. Use Ctrl+P para salvar em PDF.");
        }
      }, 120);
    });
  }
  ...
  const cards = [
    { l:"Total de manutenções", v: stats.totalManutencoes || 0, c:C.accent, i:"wrench" },
    { l:"Preventivas", v: stats.manutencoesPreventivas || 0, c:C.success, i:"check" },
    { l:"Avulsas", v: stats.manutencoesAvulsas || 0, c:C.warning, i:"alert" },
    { l:"Chamados", v: stats.totalChamados || 0, c:C.primary, i:"chat" },
    { l:"Valores investidos", v: fmt.money(stats.valoresInvestidos || 0), c:C.primary, i:"money" },
    { l:"Score preventivas", v: `${stats.scorePreventivas || 0}%`, c:C.success, i:"chart" },
    { l:"Score avulsas", v: `${stats.scoreAvulsas || 0}%`, c:C.warning, i:"chart" },
    { l:"Chamados abertos", v: stats.chamadosAbertos || 0, c:C.danger, i:"bell" },
  ];
  ...
  return (
    ...
    <div className="page no-print">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,marginBottom:22,flexWrap:"wrap"}}>
        <div>
          <h1 style={{fontFamily:"'Sora',sans-serif",fontSize:30,fontWeight:900,letterSpacing:0}}>Painel administrativo</h1>
          <p style={{color:C.muted,fontSize:14,marginTop:6}}>Bem-vindo ao painel de gestão. {data?.scope?.label || "Todas as edificações"} com dados filtrados por edificação, categoria e período.</p>
        </div>
        <button className="btn btn-ghost btn-sm" onClick={handlePrintDashboard} disabled={!data}>
          <Ic n="doc" s={15} /> Imprimir / PDF
        </button>
      </div>
      {printInfo && <div className="card" style={{padding:12,marginBottom:14,borderLeft:`4px solid ${C.primary}`,fontSize:13,color:C.muted}}>{printInfo}</div>}

      <form onSubmit={filtrar} className="card" style={{padding:16,marginBottom:18}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:12,alignItems:"end"}}>
          <div>
            <label>Edificação</label>
            <select value={draft.edificacaoId} onChange={e=>change("edificacaoId", e.target.value)}>
          ...
          <div>
            ...
            <select value={draft.categoria} onChange={e=>change("categoria", e.target.value)}>
              {categorias.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
            </select>
          </div>
          <div>
            <label>Data inicial</label>
            <input type="date" value={draft.de} onChange={e=>change("de", e.target.value)} />
          </div>
          <div>
            <label>Data final</label>
            <input type="date" value={draft.ate} onChange={e=>change("ate", e.target.value)} />
          </div>
          <div style={{display:"flex",gap:8}}>
            <button className="btn btn-primary" type="submit" style={{flex:1,justifyContent:"center"}} disabled={loading}>Filtrar</button>
            <button className="btn btn-ghost" type="button" onClick={limpar}>Limpar</button>
          </div>
        </div>
      </form>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14,marginBottom:18}}>
        {cards.map((card, i) => (
          <div key={card.l} className="stat fadeIn" style={{animationDelay:`${i*.04}s`,display:"flex",alignItems:"center",gap:13}}>
            <div style={{width:40,height:40,borderRadius:11,background:card.c+"20",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <Ic n={card.i} s={19} c={card.c} />
            </div>
            <div style={{minWidth:0}}>
              <div className="stat-n" style={{color:card.c,fontSize:24,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{card.v}</div>
              <div className="stat-l">{card.l}</div>
              <div style={{fontSize:11,color:card.c,marginTop:10,fontWeight:800}}>↑ visão atual</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {alertas.length > 0 && (
        <div className="card" style={{padding:18,borderLeft:`4px solid ${C.warning}`,marginBottom:18}}>
          <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:12}}>
            <Ic n="alert" s={17} c={C.warning} />
            <span style={{fontFamily:"'Sora',sans-serif",fontWeight:700,fontSize:14}}>Alertas de vencimento</span>
          </div>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            {alertas.map(a => (
              <div key={a.id} style={{background:"#FFF3CD",border:"1px solid #FFE69C",borderRadius:8,padding:"7px 13px",fontSize:13,color:"#856404"}}>
                {a.condominio?.nome ? `${a.condominio.nome} - ` : ""}{a.titulo} - {fmt.date(a.dataVencimento)}
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:18}}>
        <DashboardBars title="Linha do tempo de manutenções" items={graficos.timelineManutencoes || []} color={C.accent} />
        <DashboardBars title="Tipos de chamados" items={graficos.tiposChamados || []} color={C.primary} />
        <DashboardBars title="Categorias de manutenção" items={graficos.categoriasManutencao || []} color={C.success} />
        <DashboardBars title="Relatórios" items={graficos.relatorios || []} color={C.primary} />
      </div>

      <DashboardActivities items={graficos.atividadesPorUsuario || []} />
    </div>
    {printData && <DashboardPrintView data={printData} />}
    </>
  );
}
