  ...
  return (
    <div style={{width:"100%",maxWidth:430,margin:"0 auto",minHeight:"100vh",overflowX:"hidden",background:`linear-gradient(180deg,#001006 0%,${deep} 44%,#06150D 100%)`,color:"#fff"}}>
      ...
      <div style={{padding:"18px 14px 112px",marginTop:0}}>
        ...
        {tab==="contatos" && <div className="fadeIn"><h3 style={{fontFamily:"'Sora',sans-serif",fontSize:18,marginBottom:14}}>Contatos / Colaboradores</h3>{responsaveis.map(r=><div key={r.id || r.nome} className="card" style={{padding:15,marginBottom:10,borderRadius:18}}><strong style={{fontSize:14}}>{r.nome}</strong><div style={{fontSize:12,color:C.muted,marginTop:3}}>{r.funcao || r.role || "Responsável"}</div>{info.telefones && <div style={{fontSize:12,color:C.muted,marginTop:6}}>{r.telefone || r.whatsapp || "Telefone não informado"}</div>}{info.email && r.email && <div style={{fontSize:12,color:C.muted}}>{r.email}</div>}</div>)}{!responsaveis.length && <p style={{fontSize:13,color:C.muted}}>Nenhum contato liberado para o portal.</p>}</div>}
