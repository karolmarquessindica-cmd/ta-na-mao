...
import { cotacaoRouter } from './routes/cotacao.js'
...
const uploadsPath = path.resolve(__dirname, '../uploads'); fs.mkdirSync(uploadsPath, { recursive: true })
function getRedisTarget(){try{const url=new URL(process.env.REDIS_URL||'redis://localhost:6379');return{host:url.hostname||'localhost',port:parseInt(url.port)||6379}}catch{return{host:'localhost',port:6379}}}
function canReachRedis(timeoutMs=700){const{host,port}=getRedisTarget();return new Promise(resolve=>{const socket=net.createConnection({host,port});const done=ok=>{socket.removeAllListeners();socket.destroy();resolve(ok)};socket.setTimeout(timeoutMs);socket.once('connect',()=>done(true));socket.once('timeout',()=>done(false));socket.once('error',()=>done(false))})}
const corsOptions={origin:true,credentials:true,methods:['GET','POST','PUT','PATCH','DELETE','OPTIONS'],allowedHeaders:['Content-Type','Authorization','X-Requested-With']}
app.use(cors(corsOptions));app.options('*',cors(corsOptions));app.use(helmet({crossOriginResourcePolicy:false}));app.use(express.json({limit:'25mb'}));app.use(express.urlencoded({extended:true,limit:'25mb'}))
app.use('/uploads',express.static(uploadsPath,{maxAge:'1d',etag:true,fallthrough:true,setHeaders:res=>{res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Cross-Origin-Resource-Policy','cross-origin')}}))
app.use(requestId);app.use('/api',apiLimiter)
app.use('/api/auth',authRouter);app.use('/api/logo-data',logoDataPublicRouter)
app.use('/api/manutencoes',manutencaoExecucaoPublicaRouter);app.use('/api/manutencoes',manutencaoPlanejamentoRouter);app.use('/api/manutencoes',manutencaoRouter)
app.use('/api/maintenance',manutencaoExecucaoPublicaRouter);app.use('/api/maintenance',manutencaoPlanejamentoRouter);app.use('/api/maintenance',manutencaoRouter)
...
app.get('/api/health',(_,res)=>res.json({status:'ok',version:'2.0.5',portalTickets:'direct-first',cotacoes:'public-link',ts:new Date().toISOString()}));app.use(errorHandler)
app.listen(PORT,async()=>{console.log(`\n🏢 Tá na Mão API v2.0 → http://localhost:${PORT}`);await ensureBootstrapData();if(process.env.DISABLE_JOBS==='true'){process.env.JOBS_BACKEND='disabled';console.log('[boot] Jobs desativados por DISABLE_JOBS=true');return}if(await canReachRedis()){process.env.JOBS_BACKEND='bullmq';const[{iniciarWorkers},{iniciarScheduler}]=await Promise.all([import('./jobs/workers/index.js'),import('./jobs/scheduler.js')]);iniciarWorkers();iniciarScheduler().catch(e=>console.error('[boot] Falha ao iniciar scheduler:',e.message))}else{process.env.JOBS_BACKEND='native';const{iniciarNativeScheduler}=await import('./jobs/nativeScheduler.js');iniciarNativeScheduler()}})
export default app
