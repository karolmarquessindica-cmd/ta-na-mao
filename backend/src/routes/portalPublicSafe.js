...
portalPublicSafeRouter.post('/:token/chamados', uploadLimiter, multerUpload.array('fotos', 5), async (req, res, next) => { try { const condominio = await findCondominioByPortalToken(req.params.token); if (!condominio) return res.status(404).json({ error: 'Portal nao encontrado', code: 'PORTAL_NOT_FOUND' }); const portal = normalizePortalConfig(condominio.portalConfig).portalMorador; if (portal.ativo === false || portal.permitirLink === false || portal.funcionalidades?.abrirChamado === false) return res.status(403).json({ error: 'Abertura de chamados indisponivel neste portal', code: 'PORTAL_TICKET_DISABLED' }); const { categoria, descricao, local, nome, bloco, apartamento, whatsapp } = req.body; if (!descricao || !String(descricao).trim()) return res.status(400).json({ error: 'Descricao do chamado e obrigatoria', code: 'VALIDATION_ERROR' }); const files = req.files || []; await validatePortalFiles(files); const uploads = []; for (const file of files) { const { url } = await uploadFile(file, 'portal-chamados'); uploads.push(url) } const morador = await portalAnonymousUser(condominio.id); const dadosMorador = { nome: String(nome || '').trim(), bloco: String(bloco || '').trim(), apartamento: String(apartamento || '').trim(), whatsapp: String(whatsapp || '').trim(), condominioId: condominio.id, condominioNome: condominio.nome }; const chamado = await prisma.chamado.create({ data: { titulo: `Chamado pelo portal - ${condominio.nome}${local ? ` - ${local}` : ''}`, descricao, categoria: ['MANUTENCAO', 'RECLAMACAO', 'SUGESTAO'].includes(categoria) ? categoria : 'MANUTENCAO', prioridade: 'MEDIA', fotos: uploads, moradorId: morador.id, condominioId: condominio.id, historico: { create: { acao: 'Chamado aberto pelo Portal do Morador', nota: JSON.stringify({ local: local || null, dadosMorador }) } } } }); res.status(201).json({ id: chamado.id, protocolo: chamado.id.slice(0, 8).toUpperCase(), status: chamado.status, createdAt: chamado.createdAt, mensagem: TICKET_CONFIRMATION_MESSAGE }) } catch (e) { next(e) } })